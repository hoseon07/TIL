...
app.get('/update/:name', (req, res) => {
  ...
  db.query(`SELECT * FROM students where name=?`, [studentName], (err, student) => {
    if (err) {
      throw err
    }
    ...
    <input type="text" name="name_old" value="${studentName}" hidden>
      ...
      <p>설명 : 
        <textarea name="desc">${student[0].profile}</textarea>
      </p>
      <p>
        <input type="submit" value="저장">
      </p>
...
app.post('/process_update', (req, res) => {
  ...
  db.query(`UPDATE students SET name=?, profile=?, school_id=? WHERE name=?`,
    [name, desc, 1, oldName],
    (err, result) => {
      res.redirect(`/student/${name}`)
    }
...
app.post('/process_delete', (req, res) => {
  ...
  db.query(`DELETE FROM students WHERE name=?`, [studentName],
    (err, result) => {
      if (err) {
        throw err
      }
